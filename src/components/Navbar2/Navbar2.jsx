...
const Navbar2 = ({ setIsLogin }) => {
  const navigate = useNavigate();
  const user = getCurrentUser(); // get current logged-in user info

  const handleLogout = () => {
    logout();          // remove user info and login state
    setIsLogin(false); // update App state
